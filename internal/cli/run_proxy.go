...
import (
	...
	"github.com/9seconds/mtg/v2/internal/sspanel" // 新增 SSPanel 包
	...
	"github.com/9seconds/mtg/v2/secret" // 新增 secret 包
...
// 新增函数：创建 SSPanel 流量上报器
func makeSSPanelReporter(conf *config.Config, logger mtglib.Logger) events.Observer {
	if !conf.SSPanel.Enable {
		return events.NewNoopObserver()
	}

	client := sspanel.NewClient(
		conf.SSPanel.APIBaseURL,
		conf.SSPanel.NodeID,
		conf.SSPanel.APIKey,
	)
	
	reporter := events.NewSSPanelReporter(
		client,
		conf.SSPanel.TrafficBatchSize,
		time.Duration(conf.SSPanel.TrafficFlushDelay)*time.Second,
	)
	
	// 启动心跳协程
	go func() {
		ticker := time.NewTicker(time.Duration(conf.SSPanel.HeartbeatInterval) * time.Second)
		defer ticker.Stop()
...
// 新增函数：发送心跳
func sendHeartbeat(client *sspanel.Client, logger mtglib.Logger) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	
	status, err := client.SendHeartbeat(ctx)
	if err != nil {
		logger.Error("SSPanel heartbeat failed", "error", err)
	} else {
		logger.Info("SSPanel heartbeat status", "status", status)
		
		// 如果返回offline或unknown，需要检查配置
		if status == "offline" || status == "unknown" {
			logger.Warn("SSPanel reports node status as problematic", "status", status)
		}
	}
}

func makeEventStream(conf *config.Config, logger mtglib.Logger) (mtglib.EventStream, error) {
	factories := make([]events.ObserverFactory, 0, 3) // 增加容量
	
	// 添加 SSPanel 上报器
	if conf.SSPanel.Enable {
		sspanelReporter := makeSSPanelReporter(conf, logger)
		factories = append(factories, func() events.Observer { return sspanelReporter })
	}
...
// 新增函数：初始化 SSPanel 动态密钥管理器
func setupSSPanelIntegration(conf *config.Config, eventStream mtglib.EventStream, ntw mtglib.Network, logger mtglib.Logger) (mtglib.Secret, error) {
	if !conf.SSPanel.Enable {
	...
	dynamicManager := secret.NewDynamicManager(
		client,
		time.Duration(conf.SSPanel.SyncInterval)*time.Second,
	)
	...
	if err := dynamicManager.Sync(ctx); err != nil {
		logger.Warn("Initial SSPanel user sync failed", "error", err)
	} else {
		logger.Info("Initial SSPanel user sync completed", "count", dynamicManager.UserCount())
	}
	
	// 启动定期同步
	go func() {
		ticker := time.NewTicker(time.Duration(conf.SSPanel.SyncInterval) * time.Second)
		defer ticker.Stop()
		
		for range ticker.C {
			syncCtx, syncCancel := context.WithTimeout(context.Background(), 30*time.Second)
			if err := dynamicManager.Sync(syncCtx); err != nil {
				logger.Warn("SSPanel user sync failed", "error", err)
			} else {
				logger.Info("SSPanel user sync completed", "count", dynamicManager.UserCount())
				eventStream.Send(context.Background(), mtglib.NewEventSSPanelUserSync(dynamicManager.UserCount()))
			}
			syncCancel()
		}
	}()
...
func runProxy(conf *config.Config, version string) error { //nolint: funlen
	...
	// 设置 SSPanel 动态密钥
	secretManager, err := setupSSPanelIntegration(conf, eventStream, ntw, logger)
	if err != nil {
		logger.Warn("SSPanel integration failed, using static secret", "error", err)
		secretManager = conf.Secret
	} else {
		logger.Info("SSPanel dynamic secret manager initialized")
	}
	...
	opts := mtglib.ProxyOpts{
		...
		Secret:             secretManager, // 使用动态密钥管理器
