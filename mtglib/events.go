...
// EventSSPanelUserSync is emitted when we sync user list from SSPanel.
type EventSSPanelUserSync struct {
	eventBase

	UserCount int
}

// NewEventSSPanelUserSync creates a new EventSSPanelUserSync event.
func NewEventSSPanelUserSync(userCount int) EventSSPanelUserSync {
	return EventSSPanelUserSync{
		eventBase: eventBase{
			timestamp: time.Now(),
		},
		UserCount: userCount,
...
// EventSSPanelTrafficReport is emitted when we report traffic to SSPanel.
type EventSSPanelTrafficReport struct {
	eventBase
...
// NewEventSSPanelTrafficReport creates a new EventSSPanelTrafficReport event.
func NewEventSSPanelTrafficReport(success bool, count int) EventSSPanelTrafficReport {
	return EventSSPanelTrafficReport{
		eventBase: eventBase{
			timestamp: time.Now(),
		},
		Success: success,
